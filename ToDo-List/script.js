...
const listOfTasksContainer = document.getElementById("title_and_list_container");
...
addTaskButton.addEventListener("click", function () {
    ...
    if (newTaskTitle && newTaskTitle.trim().length !== 0) {
        ...
        removeTaskButton.addEventListener("click", () => {
            let confirmPrompt = window.confirm("Are you sure you want to delete this task?");
            if (confirmPrompt) {
                ListItem.remove();
...
document
    ...
    .addEventListener("click", (event) => {
        ...
        if (event.target.name === "checkbox_filterList" && event.target.value === "completed") {
        ...
        } else if (event.target.name === "checkbox_filterList" && event.target.value === "incompleted") {
            for (let checkbox of completionCheckboxes) {
                if (checkbox.checked) {
                    checkbox.parentElement.style.display = "none";
                } else {
...
function dragDrop(container, vertical) {
    const draggableElements = [...container.querySelectorAll(".draggable:not(.dragging-state)")];
